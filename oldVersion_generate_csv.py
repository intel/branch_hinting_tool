...
def applyOnFolder(folder, zend):
	...
	if zend == True:
		...
		for i in range(len(all_folders)):
			os.chdir(all_folders[i])
			path = folder + "/Zend/GCOVS/" + all_folders[i] + "/"
			dir_ls = os.listdir(".")
			""" create the csv files """
			for j in range(len(dir_ls)):
				#print path + dir_ls[i]
				if dir_ls[j].endswith(".h.gcov") or dir_ls[j].endswith(".c.gcov"):
					generate(dir_ls[j])
			
			dir_ls = os.listdir(".")
			#print dir_ls
			""" sum up all csv files into one """
			m = calculs.Merge()
			for j in range(len(dir_ls)):
				if dir_ls[j].endswith(".csv") :
					#print path + dir_ls[j]
					m.addFile(dir_ls[j])
			m.writeToFile(all_folders[i] + "_sum_.csv") 
			os.chdir("..")
		os.chdir(old_path)
	else:
