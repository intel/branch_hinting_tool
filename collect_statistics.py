    ...
    def to_string(self):
        return "%s line %s :\n\tType: %s\n\tTimesTaken : \n\t\tBranch 0: %s" + \
            "\n\t\tBranch1: %s\n\tPercent times taken: %s%%\n" % (
                ...
                "UNEXPECTED" if self.type else "EXPECTED",
                self.branch0, self.branch1, self.proc)
...
class Collector(object):
    ...
    def write_csv(self, filename):
        ...
        try:
            ...
            raise
        wfile.write("PATH, FILENAME, LINE,ORIGINAL_LINE_NO, STATE," \
            " CURRENT HINT, EXPECTED HINT, TOTAL #, TAKEN % , TAKEN," \
            "  NOT TAKEN, NUM_BRANCHES, BRANCH TYPE, LINE OF CODE\n")

        for key in self.map:
            value = self.map[key]
            wfile.write(",".join(str(item) for item in ([
                value[10], key, value[11]] + value[0:9] + [
                    str(value[9]).split(',')[0]])) + '\n')
