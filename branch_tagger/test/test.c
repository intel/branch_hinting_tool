...
trep combine_arr_bits(trep a, trep b) {
  ...
  // 2 bit cases:
  if (arr_part == (BSArrN|BCArrE))/*if branch &&*/ return static_cast<trep>(combined|BArr);
  if (arr_part == (BSArrE|BCArrN))/*if branch &&*/ return static_cast<trep>(combined|BArr);
  // 3 bit cases:
  if (arr_part == (BArrN|BCArrE))/*if branch &&*/ return static_cast<trep>(combined|BArr);
  if (arr_part == (BArrN|BSArrE))/*if branch &&*/ return static_cast<trep>(combined|BArr);
  if (arr_part == (BArrE|BCArrN))/*if branch &&*/ return static_cast<trep>(combined|BArr);
  if (arr_part == (BArrE|BSArrN))/*if branch &&*/ return static_cast<trep>(combined|BArr);
...
folly::Optional<DArrPacked> toDArrPacked(SArray ar) {
  ...
  auto idx = size_t{0};
  for (ArrayIter iter(ar); iter; ++iter, ++idx)/*for branch &&*/ {
    auto const key = *iter.first().asTypedValue();
    if (key.m_type != KindOfInt64)/*if branch &&*/ return folly::none;
    if (key.m_data.num != idx)/*if branch &&*/     return folly::none;
    elems.push_back(
...
folly::Optional<DArrStruct> toDArrStruct(SArray ar) {
  ...
  auto map = StructMap{};
  for (ArrayIter iter(ar); iter; ++iter)/*for branch &&*/ {
    auto const key = *iter.first().asTypedValue();
    if (!isStringType(key.m_type))/*if branch &&*/ return folly::none;
...
bool subtypePacked(const DArrPacked& a, const DArrPacked& b) {
  ...
  if (asz != bsz)/*if branch &&*/ return false;
  for (auto i = size_t{0}; i < asz; ++i)/*for branch &&*/ {
    if (!a.elems[i].subtypeOf(b.elems[i]))/*if branch &&*/ {
      return false;
    }
  }
  return true;
...
bool subtypeStruct(const DArrStruct& a, const DArrStruct& b) {
  if (a.map.size() != b.map.size())/*if branch &&*/ return false;
  auto aIt = begin(a.map);
  auto bIt = begin(b.map);
  for (; aIt != end(a.map); ++aIt, ++bIt)/*for branch &&*/ {
    if (aIt->first != bIt->first)/*if branch &&*/ return false;
    if (!aIt->second.subtypeOf(bIt->second))/*if branch &&*/ return false;
  }
  return true;
}
...
bool couldBePacked(const DArrPacked& a, const DArrPacked& b) {
  ...
  if (asz != bsz)/*if branch &&*/ return false;
  for (auto i = size_t{0}; i < asz; ++i)/*for branch &&*/ {
    if (!a.elems[i].couldBe(b.elems[i]))/*if branch &&*/ {
      return false;
    }
  }
  return true;
...
bool couldBeStruct(const DArrStruct& a, const DArrStruct& b) {
  ...
  for (; aIt != end(a.map); ++aIt, ++bIt)/*for branch &&*/ {
    if (aIt->first != bIt->first)/*if branch &&*/ return false;
    if (!aIt->second.couldBe(bIt->second))/*if branch &&*/ return false;
  }
  return true;
}
...
Type struct_values(const DArrStruct& a) {
  auto ret = TBottom;
  for (auto& kv : a.map)/*for branch &&*/ ret = union_of(ret, kv.second);
  return ret;
}

Type packed_values(const DArrPacked& a) {
  auto ret = TBottom;
  for (auto& e : a.elems)/*for branch &&*/ ret = union_of(ret, e);
  return ret;
}
...
struct DisjointCouldBeImpl {
  ...
  bool operator()(const DArrPacked& a, const DArrPackedN& b) const {
    for (auto& t : a.elems)/*for branch &&*/ {
      if (!t.couldBe(b.type))/*if branch &&*/ return false;
    }
    return true;
  }
  ...
  bool operator()(const DArrStruct& a, const DArrMapN& b) const {
    if (!TSStr.couldBe(b.key))/*if branch &&*/ return false;
    for (auto& kv : a.map)/*for branch &&*/ {
      if (!kv.second.couldBe(b.val))/*if branch &&*/ return false;
    }
    return true;
  }
...
struct ArrUnionImpl {
  ...
  Type operator()(const DArrPacked& a, const DArrPacked& b) const {
    if (a.elems.size() != b.elems.size())/*if branch &&*/ {
      return arr_packedn(union_of(packed_values(a), packed_values(b)));
    }
    auto ret = a.elems;
    for (auto i = size_t{0}; i < a.elems.size(); ++i)/*for branch &&*/ {
  ...
  Type operator()(const DArrStruct& a, const DArrStruct& b) const {
     ...
     */
    if (a.map.size() != b.map.size())/*if branch &&*/ return to_map();

    auto retStruct = StructMap{};
    auto aIt = begin(a.map);
    auto bIt = begin(b.map);
    for (; aIt != end(a.map); ++aIt, ++bIt)/*for branch &&*/ {
      if (aIt->first != bIt->first)/*if branch &&*/ return to_map();
...
&& p2) /*if branch &&*/{
      ...
      if (p1)/*if branch &&*/ {
        if (auto const p2n = toDArrPackedN(b))/*if branch &&*/ {
          return (*this)(*p1, *p2n);
        }
      }
      if (p2)/*if branch &&*/ {
        if (auto const p1n = toDArrPackedN(a))/*if branch &&*/ {
          return (*this)(*p2, *p1n);
        }
      }
    }
    {
      ...
      if (s1 /*if branch &&*/
&& s2) /*if branch &&*/return (*this)(*s1, *s2);
    }
...
struct DisjointSubtype {
  ...
  bool operator()(const DArrPacked& a, const DArrMapN& b) const {
    if (!TInt.subtypeOf(b.key))/*if branch &&*/ return false;
    for (auto& v : a.elems)/*for branch &&*/ {
      if (!v.subtypeOf(b.val))/*if branch &&*/ return false;
    }
    return true;
  }

  bool operator()(const DArrStruct& a, const DArrMapN& b) const {
    if (!TSStr.subtypeOf(b.key))/*if branch &&*/ return false;
    for (auto& kv : a.map)/*for branch &&*/ {
      if (!kv.second.subtypeOf(b.val))/*if branch &&*/ return false;
    }
    return true;
  }
  ...
  bool operator()(const DArrPacked& a, const DArrPackedN& b) const {
    for (auto& t : a.elems)/*for branch &&*/ {
      if (!t.subtypeOf(b.type))/*if branch &&*/ return false;
    }
    return true;
  }
...
bool Type::equivData(const Type& o) const {
  if (m_dataTag != o.m_dataTag)/*if branch &&*/ {
...
bool Type::subtypeData(const Type& o) const {
  ...
  case DataTag::Obj:
    assert(!m_data.dobj.whType);
    assert(!o.m_data.dobj.whType);
    if (m_data.dobj.type == o.m_data.dobj.type /*if branch &&*/
&&        m_data.dobj.cls.same(o.m_data.dobj.cls)) /*if branch &&*/{
      return true;
    }
    if (o.m_data.dobj.type == DObj::Sub)/*if branch &&*/ {
      return m_data.dobj.cls.subtypeOf(o.m_data.dobj.cls);
    }
    return false;
  case DataTag::Cls:
    if (m_data.dcls.type == o.m_data.dcls.type /*if branch &&*/
&&        m_data.dcls.cls.same(o.m_data.dcls.cls)) /*if branch &&*/{
      return true;
    }
    if (o.m_data.dcls.type == DCls::Sub)/*if branch &&*/ {
...
&&        m_data.dobj.cls.same(o.m_data.dobj.cls)) /*if branch &&*/{
    ...
    if (m_data.dobj.type == DObj::Sub /*if branch ||*/
|| o.m_data.dobj.type == DObj::Sub) /*if branch &&*/{
...
&&        m_data.dcls.cls.same(o.m_data.dcls.cls)) /*if branch &&*/{
    ...
    if (m_data.dcls.type == DCls::Sub /*if branch ||*/
|| o.m_data.dcls.type == DCls::Sub) /*if branch &&*/{
...
bool Type::operator==(const Type& o) const {
  ...
  if (m_bits != o.m_bits)/*if branch &&*/ return false;
  if (hasData() != o.hasData())/*if branch &&*/ return false;
  if (!hasData() /*if branch &&*/
&& !o.hasData()) /*if branch &&*/return true;

  if (is_specialized_wait_handle(*this))/*if branch &&*/ {
    if (is_specialized_wait_handle(o))/*if branch &&*/ {
...
bool Type::subtypeOf(Type o) const {
  ...
  if (isect != m_bits)/*if branch &&*/ return false;
  if (!mayHaveData(isect))/*if branch &&*/ return true;

  // No data is always more general.
  if (!hasData() /*if branch &&*/
&& !o.hasData()) /*if branch &&*/return true;
  if (!o.hasData())/*if branch &&*/ {
    assert(hasData());
    return true;
  }
...
bool Type::couldBe(Type o) const {
  ...
  auto const isect = static_cast<trep>(m_bits & o.m_bits);
  if (isect == 0)/*if branch &&*/ return false;
  if (subtypeOf(o) /*if branch ||*/
|| o.subtypeOf(*this)) /*if branch &&*/return true;
  if (!mayHaveData(isect))/*if branch &&*/ return true;
   ...
   */
  if (is_opt(*this))/*if branch &&*/ return is_opt(o) ? true : unopt(*this).couldBe(o);
  if (is_opt(o))/*if branch &&*/     return unopt(o).couldBe(*this);
...
bool Type::checkInvariants() const {
  ...
  case DataTag::Obj:
    if (auto t = m_data.dobj.whType.get())/*if branch &&*/ {
      t->checkInvariants();
    }
    break;
  case DataTag::ArrVal:
  ...
  case DataTag::ArrPacked:
    ...
    for (DEBUG_ONLY auto& v : m_data.apacked->elems)/*for branch &&*/ {
      assert(v.subtypeOf(TInitGen));
    }
    break;
  case DataTag::ArrStruct:
    assert(!m_data.astruct->map.empty());
    for (DEBUG_ONLY auto& kv : m_data.astruct->map)/*for branch &&*/ {
...
Type aval(SArray val) {
  assert(val->isStatic());
  if (val->empty())/*if branch &&*/ return aempty();
  auto r        = Type { BSArrN };
  r.m_data.aval = val;
...
bool is_opt(Type t) {
  if (t.m_bits == BInitNull)/*if branch &&*/ return false;
  if (!t.couldBe(TInitNull))/*if branch &&*/ return false;
...
Type objcls(const Type& t) {
  if (t.subtypeOf(TObj) /*if branch &&*/
&& is_specialized_obj(t)) /*if branch &&*/{
...
folly::Optional<Cell> tv(Type t) {
  ...
  default:
    ...
    case DataTag::ArrVal:
      if ((t.m_bits & BArrN) == t.m_bits)/*if branch &&*/ {
...
Type from_hni_constraint(SString s) {
  ...
  if (!strcasecmp(p, "HH\\resource"))/*if branch &&*/ return union_of(ret, TRes);
  if (!strcasecmp(p, "HH\\bool"))/*if branch &&*/     return union_of(ret, TBool);
  if (!strcasecmp(p, "HH\\int"))/*if branch &&*/      return union_of(ret, TInt);
  if (!strcasecmp(p, "HH\\float"))/*if branch &&*/    return union_of(ret, TDbl);
  if (!strcasecmp(p, "HH\\num"))/*if branch &&*/      return union_of(ret, TNum);
  if (!strcasecmp(p, "HH\\string"))/*if branch &&*/   return union_of(ret, TStr);
  if (!strcasecmp(p, "array"))/*if branch &&*/        return union_of(ret, TArr);
  if (!strcasecmp(p, "HH\\mixed"))/*if branch &&*/    return TInitGen;
...
Type Type::unionArr(const Type& a, const Type& b) {
  ...
  if (a.m_dataTag != b.m_dataTag)/*if branch &&*/ {
...
Type union_of(Type a, Type b) {
  if (a.subtypeOf(b))/*if branch &&*/ return b;
  if (b.subtypeOf(a))/*if branch &&*/ return a;
   ...
   */
  if (is_specialized_wait_handle(a))/*if branch &&*/ {
    if (is_specialized_wait_handle(b))/*if branch &&*/ {
    ...
    if (b == TInitNull)/*if branch &&*/ return opt(a);
  }
  if (is_specialized_wait_handle(b))/*if branch &&*/ {
    if (a == TInitNull)/*if branch &&*/ return opt(b);
  }
  ...
  // exists.
  if (is_specialized_obj(a) /*if branch &&*/
&& is_specialized_obj(b)) /*if branch &&*/{
    ...
    if (t)/*if branch &&*/ return keepOpt ? opt(subObj(*t)) : subObj(*t);
    return keepOpt ? TOptObj : TObj;
  }
  if (a.strictSubtypeOf(TCls) /*if branch &&*/
&& b.strictSubtypeOf(TCls)) /*if branch &&*/{
  ...
  if (is_specialized_array(a))/*if branch &&*/ {
    if (is_specialized_array(b))/*if branch &&*/ {
      DEBUG_ONLY auto const shouldBeOpt = is_opt(a) || is_opt(b);
      auto const t = Type::unionArr(a, b);
      if (shouldBeOpt)/*if branch &&*/ assert(is_opt(t));
      return t;
    }
    if (b.subtypeOf(TOptArrE))/*if branch &&*/ {
      // Keep a's data.
      a.m_bits = combine_arr_bits(a.m_bits, b.m_bits);
      return a;
    }
    if (b.subtypeOf(TOptArr))/*if branch &&*/ {
  ...
  if (is_ref_with_inner(a) /*if branch &&*/
&& is_ref_with_inner(b)) /*if branch &&*/{
    return ref_to(union_of(*a.m_data.inner, *b.m_data.inner));
  }

#define X(y) if (a.subtypeOf(y) /*if branch &&*/\
&& b.subtypeOf(y)) /*if branch &&*/return y;
  X(TInt)
  X(TDbl)
  X(TSStr)
  X(TCStr)
   ...
   * be the result.
   */
  if (a == TInitNull /*if branch &&*/
&& canBeOptional(b.m_bits)) /*if branch &&*/return opt(b);
  if (b == TInitNull /*if branch &&*/
&& canBeOptional(a.m_bits)) /*if branch &&*/return opt(a);
...
Type promote_emptyish(Type a, Type b) {
  if (is_opt(a))/*if branch &&*/ a = unopt(a);
  if (a.subtypeOf(sempty()))/*if branch &&*/ {
    return b;
  }
  auto t = trep(a.m_bits & ~(BNull | BFalse));
  if (!isPredefined(t))/*if branch &&*/ {
    if (trep(t & BInitPrim) == t)/*if branch &&*/ {
      t = BInitPrim;
    } else if (trep(t & BInitUnc) == t)/*if branch &&*/ {
      t = BInitUnc;
    } else if (trep(t & BInitCell) == t)/*if branch &&*/ {
      t = BInitCell;
    } else {
      t = BInitGen;
    }
...
Type stack_flav(Type a) {
  if (a.subtypeOf(TUninit))/*if branch &&*/   return TUninit;
  if (a.subtypeOf(TInitCell))/*if branch &&*/ return TInitCell;
  if (a.subtypeOf(TRef))/*if branch &&*/      return TRef;
  if (a.subtypeOf(TGen))/*if branch &&*/      return TGen;
  if (a.subtypeOf(TCls))/*if branch &&*/      return TCls;
...
Type loosen_statics(Type a) {
  ...
  if (a.couldBe(TSStr))/*if branch &&*/ a = union_of(a, TStr);
  if (a.couldBe(TSArr))/*if branch &&*/ a = union_of(a, TArr);
  return a;
}
...
Type remove_uninit(Type t) {
  ...
  if (!t.couldBe(TUninit))/*if branch &&*/  return t;
  if (t.subtypeOf(TUninit))/*if branch &&*/ return TBottom;
  if (t.subtypeOf(TNull))/*if branch &&*/   return TInitNull;
  if (t.subtypeOf(TPrim))/*if branch &&*/   return TInitPrim;
  if (t.subtypeOf(TUnc))/*if branch &&*/    return TInitUnc;
  return TInitCell;
}
...
ArrKey disect_key(const Type& keyTy) {
  ...
  if (keyTy.strictSubtypeOf(TInt))/*if branch &&*/ {
    ret.i = keyTy.m_data.ival;
    ret.type = keyTy;
    return ret;
  }
  if (keyTy.strictSubtypeOf(TStr) /*if branch &&*/
&& keyTy.m_dataTag == DataTag::Str) /*if branch &&*/{
    ret.s = keyTy.m_data.sval;
    ret.type = keyTy;
    int64_t i;
    if (keyTy.m_data.sval->isStrictlyInteger(i))/*if branch &&*/ {
      ret.i = i;
      ret.type = TInt;
    }
    return ret;
  }
  if (keyTy.strictSubtypeOf(TDbl))/*if branch &&*/ {
    ret.i = static_cast<int64_t>(keyTy.m_data.dval);
    ret.type = TInt;
    return ret;
  }
  if (keyTy.subtypeOf(TNum))/*if branch &&*/ {
    ret.type = TInt;
    return ret;
  }

  if (keyTy.subtypeOf(TNull))/*if branch &&*/ {
    ret.s = s_empty.get();
    ret.type = sempty();
    return ret;
  }
  if (keyTy.subtypeOf(TRes))/*if branch &&*/ {
    ret.type = TInt;
    return ret;
  }

  if (keyTy.subtypeOf(TTrue))/*if branch &&*/ {
    ret.i = 1;
    ret.type = TInt;
    return ret;
  }
  if (keyTy.subtypeOf(TFalse))/*if branch &&*/ {
    ret.i = 0;
    ret.type = TInt;
    return ret;
  }
  if (keyTy.subtypeOf(TBool))/*if branch &&*/ {
    ret.type = TInt;
    return ret;
  }
  ...
  if (keyTy.strictSubtypeOf(TOptStr) /*if branch &&*/
&& keyTy.m_dataTag == DataTag::Str) /*if branch &&*/{
    int64_t ignore;
    if (!keyTy.m_data.sval->isStrictlyInteger(ignore))/*if branch &&*/ {
...
Type array_elem(const Type& arr, const Type& undisectedKey) {
  ...
  auto ty = [&]() -> Type {
    ...
    case DataTag::ArrVal:
      if (key.i)/*if branch &&*/ {
        if (auto const r = arr.m_data.aval->nvGet(*key.i))/*if branch &&*/ {
          return from_cell(*r);
        }
        return TInitNull;
      }
      if (key.s)/*if branch &&*/ {
        if (auto const r = arr.m_data.aval->nvGet(*key.s))/*if branch &&*/ {
          return from_cell(*r);
        }
        return TInitNull;
      }
    ...
    case DataTag::ArrPacked:
      ...
      if (*key.i >= 0 /*if branch &&*/
&& *key.i < arr.m_data.apacked->elems.size()) /*if branch &&*/{
  ...
  if (!ty.subtypeOf(TInitCell))/*if branch &&*/ {
    ty = TInitCell;
  } else if (arr.couldBe(TArrE))/*if branch &&*/ {
...
Type arrayN_set(Type arr, const Type& undisectedKey, const Type& val) {
  ...
  auto ensure_counted = [&] {
    ...
    if (arr.m_bits & BCArr)/*if branch &&*/ return;
    if (arr.m_bits == BSArrN)/*if branch &&*/ {
      arr.m_bits = combine_arr_bits(arr.m_bits, BCArrN);
    } else if (arr.m_bits == BSArrE)/*if branch &&*/ {
  ...
  case DataTag::ArrVal:
    {
      if (auto d = toDArrStruct(arr.m_data.aval))/*if branch &&*/ {
        return arrayN_set(arr_struct(std::move(d->map)), undisectedKey, val);
      }
      if (auto d = toDArrPacked(arr.m_data.aval))/*if branch &&*/ {
        return arrayN_set(arr_packed(std::move(d->elems)), undisectedKey,
                          val);
      }
      if (auto d = toDArrPackedN(arr.m_data.aval))/*if branch &&*/ {
...
&& *key.i < arr.m_data.apacked->elems.size()) /*if branch &&*/{
        ...
        if (current.subtypeOf(TInitCell))/*if branch &&*/ {
          current = val;
        }
        return arr;
      }
      if (*key.i == arr.m_data.apacked->elems.size())/*if branch &&*/ {
  ...
  case DataTag::ArrStruct:
    ensure_counted();
    {
      ...
      if (key.s)/*if branch &&*/ {
        ...
        if (it == end(map))/*if branch &&*/ {
          map[*key.s] = val;
        } else {
          if (it->second.subtypeOf(TInitCell))/*if branch &&*/ {
            it->second = val;
          }
        }
        return arr;
...
Type array_set(Type arr, const Type& undisectedKey, const Type& val) {
  ...
  auto nonEmptyPart =
    arr.couldBe(TArrN) ? arrayN_set(arr, undisectedKey, val)
                       : TBottom;
  if (!arr.couldBe(TArrE))/*if branch &&*/ {
...
&& !key.i) /*if branch &&*/{
    ...
    if (key.i /*if branch &&*/
&& *key.i == 0) /*if branch &&*/return arr_packed({val});
...
std::pair<Type,Type> iter_types(const Type& iterable) {
  if (!iterable.subtypeOf(TArr) /*if branch ||*/
|| !is_specialized_array(iterable)) /*if branch &&*/{
  ...
  case DataTag::None:
    if (iterable.subtypeOf(TSArr))/*if branch &&*/ {
...
RepoAuthType make_repo_type_arr(ArrayTypeTable::Builder& arrTable,
  ...
  auto const tag = [&]() -> RepoAuthType::Tag {
    if (t.subtypeOf(TSArr))/*if branch &&*/    return RepoAuthType::Tag::SArr;
    if (t.subtypeOf(TArr))/*if branch &&*/     return RepoAuthType::Tag::Arr;
    if (t.subtypeOf(TOptSArr))/*if branch &&*/ return RepoAuthType::Tag::OptSArr;
    if (t.subtypeOf(TOptArr))/*if branch &&*/  return RepoAuthType::Tag::OptArr;
    not_reached();
  }();
...
RepoAuthType make_repo_type(ArrayTypeTable::Builder& arrTable, const Type& t) {
  ...
  if (t.strictSubtypeOf(TObj) /*if branch ||*/
|| (is_opt(t) /*if branch &&*/
&& t.strictSubtypeOf(TOptObj))) /*if branch &&*/{
